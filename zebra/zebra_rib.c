...
COVERAGE_DEFINE(zebra_rib_cnt);
VLOG_DEFINE_THIS_MODULE(zebra_rib);
      ...
      for (rn = route_top (shadow_table); rn; rn = route_next (rn))
        {
          ...
          /*
           * Clean-up the next-hops which are in kernel but not in
           * OVSDB after zebra process restart.
           */
 ...
 * learnt from OVSDB. This function returns true only in case if after
 * restart kernel learned zebra route is same as the OVSDB learned
 * route.
 */
static bool compare_route_node_with_shadow_route_node (struct route_node* rn,
                                                       struct rib* rib)
{
  struct route_table *shadow_table;
      ...
      if (if_kernel_and_current_same)
        {
           ...
           /*
            * Mark the current rib next-hops as present in FIB,
            * as we do not want to reprogram them again after coming
            * up from restart.
            */
           current_nexthop_ptr = current_rib->nexthop;
           while (current_nexthop_ptr)
             {
                if (CHECK_FLAG(current_nexthop_ptr->flags,
                                         NEXTHOP_FLAG_ACTIVE))
                  SET_FLAG (current_nexthop_ptr->flags,
                                                NEXTHOP_FLAG_FIB);
...
#ifdef ENABLE_OVSDB
  /*
   * Create an IDL transaction to commit any route changes into OVSDB.
   * This transaction is created from the worker tread which processes
   * routing updates from the routing protocol clients.
   */
  zebra_create_txn();
#endif
  ...
  if (!(mq->size))
    {
      ...
      /*
       * Since there are no further routes to process for this
       * worker thread, we should submit all the outstanding
    ...
    {
      /*
       * Since there are further routes to process for this
       * worker thread, we should try to see if there are i
