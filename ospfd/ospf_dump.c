...
#ifdef ENABLE_OVSDB
/*
 * function for displaying debug configurations
 */
void
show_debug_info(char *buf, int buflen)
{
    char arr[1000];
    char *NEW_LINE = "\n";
    int i;
...
ospf_debug(char *buf, char* err_str, int argc, const char *argv[])
{
    ...
    if (argc >= 3  && !strncmp (argv[2], PACKET_STR, strlen(argv[2])))
    {
         /* Check packet type. */
        if (argc >= 4 && strncmp (argv[3], "h", 1) == 0)
            type = OSPF_DEBUG_HELLO;
        else if (argc >= 4 && strncmp (argv[3], "d", 1) == 0)
            type = OSPF_DEBUG_DB_DESC;
        else if (argc >= 4 && strncmp (argv[3], "ls-r", 4) == 0)
            type = OSPF_DEBUG_LS_REQ;
        else if (argc >= 4 && strncmp (argv[3], "ls-u", 4) == 0)
            type = OSPF_DEBUG_LS_UPD;
        else if (argc >= 4 && strncmp (argv[3], "ls-a", 4) == 0)
            type = OSPF_DEBUG_LS_ACK;
        else if (argc >= 4 && strncmp (argv[3], "a", 1) == 0)
            type = OSPF_DEBUG_ALL;
    ...
    else if (argc >= 3  && !strncmp (argv[2], ISM_STR, strlen(argv[2])))
    {
        /* ENABLE_NODE. */
        if (argc == 3)
            TERM_DEBUG_ON (ism, ISM);
    ...
    else if (argc >= 3  && !strncmp (argv[2], LSA_STR, strlen(argv[2])))
    {
        /* ENABLE_NODE. */
        if (argc == 3)
            TERM_DEBUG_ON (lsa, LSA);
    ...
    else if (argc >= 3  && !strncmp (argv[2], NSM_STR, strlen(argv[2])))
    {
        /* ENABLE_NODE. */
        if (argc == 3)
            TERM_DEBUG_ON (nsm, NSM);
    ...
    else if (argc >= 3  && !strncmp (argv[2], NSSA_STR, strlen(argv[2])))
    {
        TERM_DEBUG_ON (nssa, NSSA);
    }
    else if (argc >= 3  && !strncmp (argv[2], EVENT_STR, strlen(argv[2])))
    {
        TERM_DEBUG_ON (event, EVENT);
    }
    else if (argc >= 3  && !strncmp (argv[2], INTF_STR, strlen(argv[2])))
    {
        TERM_DEBUG_ON (zebra, ZEBRA_INTERFACE);
    }
    else if (argc >= 3  && !strncmp (argv[2], REDST_STR, strlen(argv[2])))
    {
        TERM_DEBUG_ON (zebra, ZEBRA_REDISTRIBUTE);
    }
    else
