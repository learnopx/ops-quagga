...
nsm_packet_received (struct ospf_neighbor *nbr)
{
  ...
  if (nbr->oi->type == OSPF_IFTYPE_NBMA && nbr->nbr_nbma)
   ...
   ovsdb_ospf_set_dead_time_intervals (nbr->oi->ifp->name,OSPF_TIME_INTERVAL_DEAD_DUE,dead_due_at,nbr->src);
#endif
...
nsm_change_state (struct ospf_neighbor *nbr, int state)
{
  ...
  if ((old_state != NSM_Full && state == NSM_Full) ||
      ...
      if (state == NSM_Full)
    {
      ...
      oi->area->full_nbrs++;
#ifdef ENABLE_OVSDB
      ovsdb_ospf_update_full_nbr_count (nbr,oi->area->full_nbrs);
#endif
