...
def add_static_routes(**kwargs):
    ...
    LogOutput('info', "\n\n\n######### Configuring switch 1 IP static routes "
              "#########")
    ...
    # Verify route 143.0.0.1/32 and next-hops in RIB, FIB and
    # running-config
    ...
    # Verify route 163.0.0.1/32 and next-hops in RIB, FIB and
    # running-config
    ...
    # Verify route 1234:1234::1/128 and next-hops in RIB, FIB and
    # running-config
    ...
    # Verify route 2234:2234::1/128 and next-hops in RIB, FIB and
    # running-config
...
def no_routing_trigger_static_routes(**kwargs):
    ...
    LogOutput('info', "Entering interface for link 1 SW1, to execute "
              "'no routing'")

    # Make the L3 interface 1 as L2 by executing "no routing"
    retStruct = InterfaceIpConfig(deviceObj=switch1,
                                  interface=switch1.linkPortMapping['lnk01'],
                                  config=False, routing='routing')
    retCode = retStruct.returnCode()
    if retCode != 0:
        assert "Failed to configure 'no rotuing' on interface 1"

    LogOutput('info', "Entering interface for link 4 SW1, to execute "
              "'no routing'")
    ...
    retCode = retStruct.returnCode()
    if retCode != 0:
        assert "Failed to configure 'no rotuing' on interface 4"
    ...
    LogOutput('info', "\n\n\n######### Verifying the IPv6 static routes "
              "on switch 1#########")
    ...
    LogOutput('info', "\n\n\n######### No routing trigger and verification "
              "of IPv4 and IPv6 static routes on switch 1 passed#########")
...
def routing_trigger_static_routes(**kwargs):
    ...
    retStruct = InterfaceIpConfig(deviceObj=switch1,
                                  interface=switch1.linkPortMapping['lnk01'],
                                  config=True, routing='routing')
    retCode = retStruct.returnCode()
    if retCode != 0:
        assert "Failed to configure 'rotuing' on interface 1"
    ...
    retCode = retStruct.returnCode()
    if retCode != 0:
        assert "Failed to configure 'rotuing' on interface 4"
    ...
    LogOutput('info', "\n\n\n######### Routing trigger and verification of "
...
class Test_ecmp_no_routing_trigger:
    def setup_class(cls):
        ...
        Test_ecmp_no_routing_trigger.testObj = testEnviron(
                                                topoDict=topoDict,
                                                defSwitchContext="vtyShell")
        # Get topology object
        Test_ecmp_no_routing_trigger.topoObj = Test_ecmp_no_routing_trigger.testObj.topoObjGet()

    def teardown_class(cls):
        Test_ecmp_no_routing_trigger.topoObj.terminate_nodes()
    ...
    def test_no_routing_trigger_static_routes(self):
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        no_routing_trigger_static_routes(switch1=dut01Obj, switch2=dut02Obj)

    def test_routing_trigger_static_routes(self):
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        routing_trigger_static_routes(switch1=dut01Obj, switch2=dut02Obj)
