...
**		        IMPORTANT
**
** This file constitues all the backend functions which are
** needed to do actual work in the bgp daemon itself.
** Note that config changes are written into the database
** and the daemon detects the database changes and performs
** the requested configuation changes by actually calling
...
static int
log_error (char *file, char *function, int line, char *fmt, ...)
{
    va_list args;
    char buffer [512];
    int c;

    c = sprintf(buffer, "%s/%s: %d, ", file, function, line);

    va_start(args, fmt);
    vsprintf(&buffer[c], fmt, args);
    va_end(args);
    VLOG_ERR(buffer);
...
#define LOG_ERROR(fmt, args...) \
    log_error((char*) __FILE__, (char*) __FUNCTION__, __LINE__, fmt, ## args)
...
peer_lookup_vty (struct vty *vty, const char *ip_str)
{
  ...
  if (ret < 0)
    {
      LOG_ERROR("%% Malformed address %s", ip_str);
      return NULL;
    }
  ...
  if (! peer)
    {
      LOG_ERROR("%% Specify remote-as or peer-group commands first");
      return NULL;
    }
  return peer;
}
       ...
       "BGP multiple instance\n")
{
  ...
  if (ret < 0)
    {
      LOG_ERROR("%% There are more than two BGP instances");
      return CMD_WARNING;
    }
  return CMD_SUCCESS;
}
       ...
       AS_STR)
{
  ...
  switch (ret)
    ...
    case BGP_ERR_MULTIPLE_INSTANCE_NOT_SET:
      LOG_ERROR("Please specify 'bgp multiple-instance' first");
      return CMD_WARNING;
    case BGP_ERR_AS_MISMATCH:
      LOG_ERROR("BGP is already running; AS is %u", as);
      return CMD_WARNING;
    case BGP_ERR_INSTANCE_MISMATCH:
      LOG_ERROR("BGP view name and AS number mismatch");
      LOG_ERROR("BGP instance is already running; AS is %u", as);
      return CMD_WARNING;
    }
       ...
       AS_STR)
{
  ...
  if (! bgp)
    {
      LOG_ERROR("%% Can't find BGP instance");
      return CMD_WARNING;
    }
       ...
       "Manually configured router identifier\n")
{
  ...
  if (! ret)
    {
      LOG_ERROR("%% Malformed bgp router identifier");
      return CMD_WARNING;
    }
       ...
       "Override configured router identifier\n")
{
  ...
  if (argc == 1)
	...
	{
	  LOG_ERROR("%% BGP router-id doesn't match");
	  return CMD_WARNING;
	}
    }
       ...
       "Route-Reflector Cluster-id in IP address format\n")
{
  ...
  if (! ret)
    {
      LOG_ERROR("%% Malformed bgp cluster identifier");
      return CMD_WARNING;
    }
  ...
  for (i = 0; i < argc; i++)
    {
      ...
      if (bgp->as == as)
	{
	  LOG_ERROR("%% Local member-AS not allowed in confed peer list");
	  continue;
	}
       ...
       "Number of paths\n")
{
  ...
  if (ret < 0)
    {
      LOG_ERROR("%% Failed to set maximum-paths %u for afi %u, safi %u",
	       maxpaths, bgp_node_afi (vty), bgp_node_safi(vty));
      return CMD_WARNING;
    }
			   ...
			   safi_t safi, int *write)
{
  ...
  if (bgp->maxpaths[afi][safi].maxpaths_ibgp != BGP_DEFAULT_MAXPATHS)
    {
      bgp_config_write_family_header (vty, afi, safi, write);
      LOG_ERROR(" maximum-paths ibgp %d%s",
       ...
       "Holdtime\n")
{
  ...
  if (holdtime < 3 && holdtime != 0)
    {
      LOG_ERROR("%% hold time value must be either 0 or greater than 3");
      return CMD_WARNING;
    }
       ...
       "peer-group name\n")
{
  ...
  if (ret < 0)
    {
      LOG_ERROR("%% Malformed address: %s%s", argv[0]);
      return CMD_WARNING;
    }
  ...
  if (ret == BGP_ERR_PEER_GROUP_PEER_TYPE_DIFFERENT)
    {
      LOG_ERROR("%% Peer with AS %u cannot be in this peer-group, members must be all internal or all external%s", as);
      return CMD_WARNING;
    }
...
static int
peer_flag_modify_bgp (struct bgp *bgp, const char *name,
                      u_int16_t flag, int set)
{
    int ret;
    struct peer *peer;
...
int
daemon_neighbor_aspath_filter_cmd_execute(struct bgp *bgp, char *peer_str,
                                          afi_t afi, safi_t safi,
                                          const char *name_str,
                                          const char *direct_str)
{
    ...
    if (name_str && direct_str) {
        VLOG_DBG("neighbor %s filter-list %s %s\n",
                 peer_str, name_str, direct_str);

        return peer_aslist_set(peer, afi, safi, direct, name_str);
    } else {
        return peer_aslist_unset(peer, afi, safi, direct);
    }
}
                            ...
                            const char *ttl_str)
{
  ...
  return log_bgp_error( peer_ebgp_multihop_set (peer, ttl));
}
...
peer_ebgp_multihop_unset_vty (struct vty *vty, const char *ip_str)
{
  ...
  return log_bgp_error(peer_ebgp_multihop_unset (peer));
}
                     ...
                     const char *weight_str)
{
  ...
  return log_bgp_error(peer_weight_set (peer, weight));
}
...
peer_weight_unset_vty (struct vty *vty, const char *ip_str)
{
  ...
  return log_bgp_error(peer_weight_unset (peer));
}
                             ...
                             const char *time_str)
{
  ...
  return log_bgp_error(peer_timers_connect_set (peer, connect));
}
...
peer_timers_connect_unset_vty (struct vty *vty, const char *ip_str)
{
  ...
  return log_bgp_error(peer_timers_connect_unset (peer));
}
       ...
       "Specify the maximum number of hops to the BGP peer\n")
{
  ...
  return log_bgp_error(peer_ttl_security_hops_set (peer, gtsm_hops));
}
       ...
       "Specify the maximum number of hops to the BGP peer\n")
{
  ...
  return log_bgp_error(peer_ttl_security_hops_unset (peer));
}
		     ...
		     safi_t safi, int error)
{
  switch (error)
    ...
    case BGP_ERR_SOFT_RECONFIG_UNCONFIGURED:
      LOG_ERROR("%%BGP: Inbound soft reconfig for %s not possible as it%s      has neither refresh capability, nor inbound soft reconfig%s", peer->host);
      break;
    default:
      break;
    }
           ...
           enum clear_sort sort,enum bgp_clear_type stype, const char *arg)
{
  ...
  if (sort == clear_peer)
    {
      ...
      if (ret < 0)
	{
	  LOG_ERROR("Malformed address: %s%s", arg);
	  return CMD_WARNING;
	}
      peer = peer_lookup (bgp, &su);
      if (! peer)
	{
	  LOG_ERROR("%%BGP: Unknown neighbor - \"%s\"%s", arg);
	  return CMD_WARNING;
	}
  ...
  if (sort == clear_group)
    {
      ...
      if (! group)
	{
	  LOG_ERROR("%%BGP: No such peer-group %s%s", arg);
	  return CMD_WARNING;
	}
  ...
  if (sort == clear_as)
      ...
      for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
	{
      ...
      if (! find)
	LOG_ERROR("%%BGP: No peer is configured with AS %s", arg);
        return CMD_SUCCESS;
    }
  ...
  if (name)
    {
      ...
      if (bgp == NULL)
        {
          LOG_ERROR("Can't find BGP view %s%s", name);
          return CMD_WARNING;
        }
    }
  else
    {
      ...
      if (bgp == NULL)
        {
          LOG_ERROR("No BGP process is configured");
          return CMD_WARNING;
        }
    }
       ...
       "Show the defined BGP views\n")
{
  ...
  if (!bgp_option_check (BGP_OPT_MULTIPLE_INSTANCE))
    {
      LOG_ERROR("Multiple BGP views are not defined");
      return CMD_WARNING;
    }

  LOG_ERROR("Defined BGP views:");
       ...
       "Global BGP memory statistics\n")
{
  ...
  LOG_ERROR("%ld RIB nodes, using %s of memory%s", count,
           mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct bgp_node)));
  count = mtype_stats_alloc (MTYPE_BGP_ROUTE);
  LOG_ERROR("%ld BGP routes, using %s of memory%s", count,
           mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct bgp_info)));
  if ((count = mtype_stats_alloc (MTYPE_BGP_ROUTE_EXTRA)))
    LOG_ERROR("%ld BGP route ancillaries, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct bgp_info_extra)));

  if ((count = mtype_stats_alloc (MTYPE_BGP_STATIC)))
    LOG_ERROR("%ld Static routes, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct bgp_static)));

  /* Adj-In/Out */
  if ((count = mtype_stats_alloc (MTYPE_BGP_ADJ_IN)))
    LOG_ERROR("%ld Adj-In entries, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct bgp_adj_in)));
  if ((count = mtype_stats_alloc (MTYPE_BGP_ADJ_OUT)))
    LOG_ERROR("%ld Adj-Out entries, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct bgp_adj_out)));
  if ((count = mtype_stats_alloc (MTYPE_BGP_NEXTHOP_CACHE)))
    LOG_ERROR("%ld Nexthop cache entries, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct bgp_nexthop_cache)));
  if ((count = mtype_stats_alloc (MTYPE_BGP_DAMP_INFO)))
    LOG_ERROR("%ld Dampening entries, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct bgp_damp_info)));

  /* Attributes */
  count = attr_count();
  LOG_ERROR("%ld BGP attributes, using %s of memory%s", count,
           mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof(struct attr)));
  if ((count = mtype_stats_alloc (MTYPE_ATTR_EXTRA)))
    LOG_ERROR("%ld BGP extra attributes, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof(struct attr_extra)));
  if ((count = attr_unknown_count()))
    LOG_ERROR("%ld unknown attributes%s", count);

  /* AS_PATH attributes */
  count = aspath_count ();
  LOG_ERROR("%ld BGP AS-PATH entries, using %s of memory", count,
           mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct aspath)));
  count = mtype_stats_alloc (MTYPE_AS_SEG);
  LOG_ERROR("%ld BGP AS-PATH segments, using %s of memory", count,
           mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct assegment)));

  /* Other attributes */
  if ((count = community_count ()))
    LOG_ERROR("%ld BGP community entries, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
  ...
  if ((count = mtype_stats_alloc (MTYPE_CLUSTER)))
    LOG_ERROR("%ld Cluster lists, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct cluster_list)));

  /* Peer related usage */
  count = mtype_stats_alloc (MTYPE_BGP_PEER);
  LOG_ERROR("%ld peers, using %s of memory%s", count,
           mtype_memstr (memstrbuf, sizeof (memstrbuf),
                         count * sizeof (struct peer)));
  if ((count = mtype_stats_alloc (MTYPE_PEER_GROUP)))
    LOG_ERROR("%ld peer groups, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct peer_group)));

  /* Other */
  if ((count = mtype_stats_alloc (MTYPE_HASH)))
    LOG_ERROR("%ld hash tables, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct hash)));
  if ((count = mtype_stats_alloc (MTYPE_HASH_BACKET)))
    LOG_ERROR("%ld hash buckets, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct hash_backet)));
  if ((count = mtype_stats_alloc (MTYPE_BGP_REGEXP)))
    LOG_ERROR("%ld compiled regexes, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
...
bgp_show_summary (struct vty *vty, struct bgp *bgp, int afi, int safi)
{
  ...
  for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
      ...
      if (peer->afc[afi][safi])
	{
          if (!count)
            {
              ...
              LOG_ERROR("RIB entries %ld, using %s of memory%s", ents,
                       mtype_memstr (memstrbuf, sizeof (memstrbuf),
                                     ents * sizeof (struct bgp_node)));

              /* Peer related usage */
              ents = listcount (bgp->peer);
              LOG_ERROR("Peers %ld, using %s of memory%s",
                       ents,
                       mtype_memstr (memstrbuf, sizeof (memstrbuf),
                                     ents * sizeof (struct peer)));

              if ((ents = listcount (bgp->rsclient)))
                LOG_ERROR("RS-Client peers %ld, using %s of memory%s",
                         ents,
                         mtype_memstr (memstrbuf, sizeof (memstrbuf),
                                       ents * sizeof (struct peer)));

              if ((ents = listcount (bgp->group)))
                LOG_ERROR("Peer groups %ld, using %s of memory%s", ents,
                         mtype_memstr (memstrbuf, sizeof (memstrbuf),
                                       ents * sizeof (struct peer_group)));

              if (CHECK_FLAG (bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING))
                LOG_ERROR("Dampening enabled.");
              LOG_ERROR("");
              LOG_ERROR("%s%s", header);
            }

	  count++;

	  len = LOG_ERROR("%s", peer->host);
	  len = 16 - len;
	  if (len < 1)
	    LOG_ERROR("%s%*s", 16, " ");
	  else
	  ...
	  if (peer->status == Established)
	    {
	      LOG_ERROR(" %8ld", peer->pcount[afi][safi]);
	    }
	  else
	    {
	      if (CHECK_FLAG (peer->flags, PEER_FLAG_SHUTDOWN))
		LOG_ERROR(" Idle (Admin)");
	      else if (CHECK_FLAG (peer->sflags, PEER_STATUS_PREFIX_OVERFLOW))
		LOG_ERROR(" Idle (PfxCt)");
	      else
		LOG_ERROR(" %-11s", LOOKUP(bgp_status_msg, peer->status));
	    }
  ...
  if (count)
    LOG_ERROR("%sTotal number of neighbors %d%s",
	     count);
  else
    LOG_ERROR("No %s neighbor is configured%s",
  ...
  if (name)
    {
      ...
      if (! bgp)
	{
	  LOG_ERROR("%% No such BGP instance exist");
	  return CMD_WARNING;
	}
       ...
       "Summary of BGP neighbor status\n")
{
  ...
  if (! ret)
    {
      LOG_ERROR("%% Malformed Route Distinguisher");
      return CMD_WARNING;
    }
...
bgp_show_peer_afi (struct vty *vty, struct peer *p, afi_t afi, safi_t safi)
{
  ...
  LOG_ERROR(" For address family: %s", afi_safi_print (afi, safi));

  if (p->af_group[afi][safi])
    LOG_ERROR("  %s peer-group member%s", p->group->name);
  ...
  if (CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_ADV)
    ...
    LOG_ERROR("  AF-dependant capabilities:");
  ...
  if (CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_ADV)
      ...
      || CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_RCV))
    {
      LOG_ERROR("    Outbound Route Filter (ORF) type (%d) Prefix-list:%s",
	       ORF_TYPE_PREFIX);
  ...
  if (CHECK_FLAG (p->af_sflags[afi][safi], PEER_STATUS_ORF_PREFIX_SEND)
      || orf_pfx_count)
    {
      LOG_ERROR("  Outbound Route Filter (ORF):");
      if (CHECK_FLAG (p->af_sflags[afi][safi], PEER_STATUS_ORF_PREFIX_SEND))
	  LOG_ERROR(" sent;");
      if (orf_pfx_count)
	LOG_ERROR(" received (%d entries)", orf_pfx_count);
      LOG_ERROR("");
    }
  if (CHECK_FLAG (p->af_sflags[afi][safi], PEER_STATUS_ORF_WAIT_REFRESH))
      LOG_ERROR("  First update is deferred until ORF or ROUTE-REFRESH is received");

  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_REFLECTOR_CLIENT))
    LOG_ERROR("  Route-Reflector Client");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
    LOG_ERROR("  Route-Server Client");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG))
    LOG_ERROR("  Inbound soft reconfiguration allowed");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_REMOVE_PRIVATE_AS))
    LOG_ERROR("  Private AS number removed from updates to this neighbor");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_NEXTHOP_SELF))
    LOG_ERROR("  NEXT_HOP is always this router");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED))
    LOG_ERROR("  AS_PATH is propagated unchanged to this neighbor");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED))
    LOG_ERROR("  NEXT_HOP is propagated unchanged to this neighbor");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_MED_UNCHANGED))
    LOG_ERROR("  MED is propagated unchanged to this neighbor");
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_SEND_COMMUNITY)
      || CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_SEND_EXT_COMMUNITY))
    {
      LOG_ERROR("  Community attribute sent to this neighbor");
      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_SEND_COMMUNITY)
	&& CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_SEND_EXT_COMMUNITY))
	LOG_ERROR("(both)");
  ...
  if (filter->plist[FILTER_IN].name
    ...
    LOG_ERROR("  Inbound path policy configured");
  if (filter->plist[FILTER_OUT].name
      || filter->dlist[FILTER_OUT].name
      || filter->aslist[FILTER_OUT].name
      || filter->map[RMAP_OUT].name
      || filter->usmap.name)
    LOG_ERROR("  Outbound path policy configured");
  if (filter->map[RMAP_IMPORT].name)
    LOG_ERROR("  Import policy for this RS-client configured");
  if (filter->map[RMAP_EXPORT].name)
    LOG_ERROR("  Export policy for this RS-client configured");

  /* prefix-list */
  if (filter->plist[FILTER_IN].name)
    LOG_ERROR("  Incoming update prefix filter list is %s%s",
	     filter->plist[FILTER_IN].plist ? "*" : "",
	     filter->plist[FILTER_IN].name);
  if (filter->plist[FILTER_OUT].name)
    LOG_ERROR("  Outgoing update prefix filter list is %s%s",
	     filter->plist[FILTER_OUT].plist ? "*" : "",
	     filter->plist[FILTER_OUT].name);

  /* distribute-list */
  if (filter->dlist[FILTER_IN].name)
    LOG_ERROR("  Incoming update network filter list is %s%s",
	     filter->dlist[FILTER_IN].alist ? "*" : "",
	     filter->dlist[FILTER_IN].name);
  if (filter->dlist[FILTER_OUT].name)
    LOG_ERROR("  Outgoing update network filter list is %s%s",
	     filter->dlist[FILTER_OUT].alist ? "*" : "",
	     filter->dlist[FILTER_OUT].name);

  /* filter-list. */
  if (filter->aslist[FILTER_IN].name)
    LOG_ERROR("  Incoming update AS path filter list is %s%s",
	     filter->aslist[FILTER_IN].aslist ? "*" : "",
	     filter->aslist[FILTER_IN].name);
  if (filter->aslist[FILTER_OUT].name)
    LOG_ERROR("  Outgoing update AS path filter list is %s%s",
	     filter->aslist[FILTER_OUT].aslist ? "*" : "",
	     filter->aslist[FILTER_OUT].name);

  /* route-map. */
  if (filter->map[RMAP_IN].name)
    LOG_ERROR("  Route map for incoming advertisements is %s%s",
            filter->map[RMAP_IN].map ? "*" : "",
            filter->map[RMAP_IN].name);
  if (filter->map[RMAP_OUT].name)
    LOG_ERROR("  Route map for outgoing advertisements is %s%s",
            filter->map[RMAP_OUT].map ? "*" : "",
            filter->map[RMAP_OUT].name);
  if (filter->map[RMAP_IMPORT].name)
    LOG_ERROR("  Route map for advertisements going into this RS-client's table is %s%s",
            filter->map[RMAP_IMPORT].map ? "*" : "",
            filter->map[RMAP_IMPORT].name);
  if (filter->map[RMAP_EXPORT].name)
    LOG_ERROR("  Route map for advertisements coming from this RS-client is %s%s",
            filter->map[RMAP_EXPORT].map ? "*" : "",
            filter->map[RMAP_EXPORT].name);

  /* unsuppress-map */
  if (filter->usmap.name)
    LOG_ERROR("  Route map for selective unsuppress is %s%s",
	     filter->usmap.map ? "*" : "",
	     filter->usmap.name);

  /* Receive prefix count */
  LOG_ERROR("  %ld accepted prefixes%s", p->pcount[afi][safi]);

  /* Maximum prefix */
  if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX))
    {
      LOG_ERROR("  Maximum prefixes allowed %ld%s%s", p->pmax[afi][safi],
	       CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING)
	       ? " (warning-only)" : "");
      LOG_ERROR("  Threshold for warning message %d%%",
	       p->pmax_threshold[afi][safi]);
      if (p->pmax_restart[afi][safi])
	LOG_ERROR(", restart interval %d min", p->pmax_restart[afi][safi]);
      LOG_ERROR("");
    }
...
bgp_show_peer (struct vty *vty, struct peer *p)
{
  ...
  LOG_ERROR("BGP neighbor is %s, ", p->host);
  LOG_ERROR("remote AS %u, ", p->as);
  LOG_ERROR("local AS %u%s%s, ",
  ...
  /* Description. */
  if (p->desc)
    LOG_ERROR(" Description: %s", p->desc);

  /* Peer-group */
  if (p->group)
    LOG_ERROR(" Member of peer-group %s for session parameters",
	     p->group->name);

  /* Administrative shutdown. */
  if (CHECK_FLAG (p->flags, PEER_FLAG_SHUTDOWN))
    LOG_ERROR(" Administratively shut down");

  /* BGP Version. */
  LOG_ERROR("  BGP version 4");
  LOG_ERROR(", remote router ID %s",
	   inet_ntop (AF_INET, &p->remote_id, buf1, BUFSIZ));

  /* Confederation */
  if (CHECK_FLAG (bgp->config, BGP_CONFIG_CONFEDERATION)
      && bgp_confederation_peers_check (bgp, p->as))
    LOG_ERROR("  Neighbor under common administration");

  /* Status. */
  LOG_ERROR("  BGP state = %s",
  ...
  if (p->status == Established)
	...
	LOG_ERROR(" (NSF passive)");
    }
  LOG_ERROR("");

  /* read timer */
  LOG_ERROR("  Last read %s", peer_uptime (p->readtime, timebuf, BGP_UPTIME_LEN));

  /* Configured timer values. */
  LOG_ERROR(", hold time is %d, keepalive interval is %d seconds%s",
	   p->v_holdtime, p->v_keepalive);
  if (CHECK_FLAG (p->config, PEER_CONFIG_TIMER))
    {
      LOG_ERROR("  Configured hold time is %d", p->holdtime);
      LOG_ERROR(", keepalive interval is %d seconds%s",
	       p->keepalive);
    }
  ...
  if (p->status == Established)
	...
	{
	  LOG_ERROR("  Neighbor capabilities:");
	  ...
	  if (CHECK_FLAG (p->cap, PEER_CAP_REFRESH_ADV)
	    ...
	    {
	      LOG_ERROR("    Route refresh:");
              if (CHECK_FLAG (p->cap, PEER_CAP_REFRESH_ADV))
		LOG_ERROR(" advertised");
	      if (CHECK_FLAG (p->cap, PEER_CAP_REFRESH_NEW_RCV)
		  || CHECK_FLAG (p->cap, PEER_CAP_REFRESH_OLD_RCV))
		LOG_ERROR(" %sreceived(%s)",
	  ...
	  for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
		...
		{
		  LOG_ERROR("    Address family %s:", afi_safi_print (afi, safi));
		  if (p->afc_adv[afi][safi])
		    LOG_ERROR(" advertised");
		  if (p->afc_recv[afi][safi])
		    LOG_ERROR(" %sreceived", p->afc_adv[afi][safi] ? "and " : "");
		  LOG_ERROR("");
		}
	  ...
	  if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_RCV)
	    ...
	    {
	      LOG_ERROR("    Graceful Restart Capabilty:");
	      ...
	      if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_RCV))
		{
		  ...
		  LOG_ERROR("      Remote Restart timer is %d seconds%s",
			   p->v_gr_restart);
		  LOG_ERROR("      Address families by peer:%s        ");

		  for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
		    for (safi = SAFI_UNICAST ; safi < SAFI_MAX ; safi++)
		      if (CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_RESTART_AF_RCV))
			{
			  LOG_ERROR("%s%s(%s)", restart_af_count ? ", " : "",
  ...
  if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_RCV)
      ...
      || p->t_gr_stale)
    {
      ...
      LOG_ERROR("  Graceful restart informations:");
      if (p->status == Established)
	{
	  LOG_ERROR("    End-of-RIB send: ");
	  for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
	    for (safi = SAFI_UNICAST ; safi < SAFI_MAX ; safi++)
	      if (CHECK_FLAG (p->af_sflags[afi][safi], PEER_STATUS_EOR_SEND))
		{
		  LOG_ERROR("%s%s", eor_send_af_count ? ", " : "",
			   afi_safi_print (afi, safi));
		  eor_send_af_count++;
		}
	  LOG_ERROR("");

	  LOG_ERROR("    End-of-RIB received: ");
	  for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
	    for (safi = SAFI_UNICAST ; safi < SAFI_MAX ; safi++)
	      if (CHECK_FLAG (p->af_sflags[afi][safi], PEER_STATUS_EOR_RECEIVED))
		{
		  LOG_ERROR("%s%s", eor_receive_af_count ? ", " : "",
			   afi_safi_print (afi, safi));
      ...
      if (p->t_gr_restart)
        LOG_ERROR("    The remaining time of restart timer is %ld%s",
                 thread_timer_remain_second (p->t_gr_restart));

      if (p->t_gr_stale)
        LOG_ERROR("    The remaining time of stalepath timer is %ld%s",
                 thread_timer_remain_second (p->t_gr_stale));
    }

  /* Packet counts. */
  LOG_ERROR("  Message statistics:");
  LOG_ERROR("    Inq depth is 0");
  LOG_ERROR("    Outq depth is %lu%s", (unsigned long) p->obuf->count);
  LOG_ERROR("                         Sent       Rcvd");
  LOG_ERROR("    Opens:         %10d %10d%s", p->open_out, p->open_in);
  LOG_ERROR("    Notifications: %10d %10d%s", p->notify_out, p->notify_in);
  LOG_ERROR("    Updates:       %10d %10d%s", p->update_out, p->update_in);
  LOG_ERROR("    Keepalives:    %10d %10d%s", p->keepalive_out, p->keepalive_in);
  LOG_ERROR("    Route Refresh: %10d %10d%s", p->refresh_out, p->refresh_in);
  LOG_ERROR("    Capability:    %10d %10d%s", p->dynamic_cap_out, p->dynamic_cap_in);
  LOG_ERROR("    Total:         %10d %10d%s", p->open_out + p->notify_out +
  ...
  LOG_ERROR("  Minimum time between advertisement runs is %d seconds%s",
	   p->v_routeadv);

  /* Update-source. */
  if (p->update_if || p->update_source)
    {
      LOG_ERROR("  Update source is ");
      if (p->update_if)
	LOG_ERROR("%s", p->update_if);
  ...
  if (CHECK_FLAG (p->config, PEER_CONFIG_WEIGHT))
    LOG_ERROR("  Default weight %d", p->weight);
  ...
  LOG_ERROR("  Connections established %d; dropped %d",
	   p->established, p->dropped);

  if (! p->dropped)
    LOG_ERROR("  Last reset never");
  else
    LOG_ERROR("  Last reset %s, due to %s%s",
            peer_uptime (p->resettime, timebuf, BGP_UPTIME_LEN),
            peer_down_str[(int) p->last_reset]);

  if (CHECK_FLAG (p->sflags, PEER_STATUS_PREFIX_OVERFLOW))
    {
      LOG_ERROR("  Peer had exceeded the max. no. of prefixes configured.");

      if (p->t_pmax_restart)
	LOG_ERROR("  Reduce the no. of prefix from %s, will restart in %ld seconds",
		 p->host, thread_timer_remain_second (p->t_pmax_restart));
      else
	LOG_ERROR("  Reduce the no. of prefix and clear ip bgp %s to restore peering%s",
		 p->host);
    }
  ...
  if (p->sort != BGP_PEER_IBGP)
    {
      if (p->gtsm_hops > 0)
	LOG_ERROR("  External BGP neighbor may be up to %d hops away.%s",
		 p->gtsm_hops);
      else if (p->ttl > 1)
	LOG_ERROR("  External BGP neighbor may be up to %d hops away.%s",
		 p->ttl);
    ...
    {
      if (p->gtsm_hops > 0)
	LOG_ERROR("  Internal BGP neighbor may be up to %d hops away.%s",
		 p->gtsm_hops);
    }
  ...
  if (p->su_local)
    {
      LOG_ERROR("Local host: %s, Local port: %d",
  ...
  if (p->su_remote)
    {
      LOG_ERROR("Foreign host: %s, Foreign port: %d",
  ...
  /* Timer information. */
  if (p->t_start)
    LOG_ERROR("Next start timer due in %ld seconds",
	     thread_timer_remain_second (p->t_start));
  if (p->t_connect)
    LOG_ERROR("Next connect timer due in %ld seconds%s",
	     thread_timer_remain_second (p->t_connect));

  LOG_ERROR("Read thread: %s  Write thread: %s",
	   p->t_read ? "on" : "off",
	   p->t_write ? "on" : "off");
  ...
  if (ip_str)
    {
      ...
      if (ret < 0)
        {
          LOG_ERROR("%% Malformed address: %s%s", ip_str);
          return CMD_WARNING;
        }
    }
...
community_show_all_iterator (struct hash_backet *backet, struct vty *vty)
{
  ...
  LOG_ERROR("[%p] (%ld) %s%s", backet, com->refcnt,
	   community_str (com));
}
...
{
  LOG_ERROR("Address Refcnt Community");
        ...
        afi_t afi, safi_t safi)
{
  ...
  LOG_ERROR("%8s", peer_uptime (rsclient->uptime, timebuf, BGP_UPTIME_LEN));
  ...
  if (CHECK_FLAG (rsclient->flags, PEER_FLAG_SHUTDOWN))
    ...
    LOG_ERROR(" %-11s", LOOKUP(bgp_status_msg, rsclient->status));
                           ...
                           afi_t afi, safi_t safi)
{
  ...
  for (ALL_LIST_ELEMENTS (bgp->rsclient, node, nnode, peer))
    {
      if (peer->afc[afi][safi] &&
         CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
       {
         if (! count)
           {
             LOG_ERROR("Route Server's BGP router identifier %s",
                      inet_ntoa (bgp->router_id));
             LOG_ERROR("Route Server's local AS number %u", bgp->as);
             LOG_ERROR("");
             LOG_ERROR("%s", header);
           }
  ...
  if (count)
    LOG_ERROR("%sTotal number of Route Server Clients %d%s",
            count);
  else
    LOG_ERROR("No %s Route Server Client is configured%s",
            afi == AFI_IP ? "IPv4" : "IPv6");
			       ...
			       safi_t safi, int *write)
{
  ...
  for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
      ...
      if (bgp->redist[afi][i] && i != ZEBRA_ROUTE_BGP)
	{
	  ...
	  LOG_ERROR(" redistribute %s", zebra_route_string(i));

	  if (bgp->redist_metric_flag[afi][i])
	    LOG_ERROR(" metric %u", bgp->redist_metric[afi][i]);

	  if (bgp->rmap[afi][i].name)
	    LOG_ERROR(" route-map %s", bgp->rmap[afi][i].name);
...
community_list_perror (struct vty *vty, int ret)
{
  switch (ret)
    ...
    case COMMUNITY_LIST_ERR_CANT_FIND_LIST:
      LOG_ERROR("%% Can't find community-list");
      break;
    case COMMUNITY_LIST_ERR_MALFORMED_VAL:
      LOG_ERROR("%% Malformed community-list value");
      break;
    case COMMUNITY_LIST_ERR_STANDARD_CONFLICT:
      LOG_ERROR("%% Community name conflict, previously defined as standard community");
      break;
    case COMMUNITY_LIST_ERR_EXPANDED_CONFLICT:
      LOG_ERROR("%% Community name conflict, previously defined as expanded community");
      break;
    }
}
                        ...
                        int style, int reject_all_digit_name)
{
  ...
  if (strncmp (argv[1], "p", 1) == 0)
    ...
    {
      LOG_ERROR("%% Matching condition must be permit or deny");
      return CMD_WARNING;
    }
  ...
  if (reject_all_digit_name && all_digit (argv[0]))
    {
      LOG_ERROR("%% Community name cannot have all digits");
      return CMD_WARNING;
    }
  ...
  for (entry = list->head; entry; entry = entry->next)
      ...
      if (entry == list->head)
	{
	  if (all_digit (list->name))
	    LOG_ERROR("Community %s list %s",
		     entry->style == COMMUNITY_LIST_STANDARD ?
		     "standard" : "(expanded) access",
		     list->name);
	  else
	    LOG_ERROR("Named Community %s list %s",
		     entry->style == COMMUNITY_LIST_STANDARD ?
		     "standard" : "expanded",
		     list->name);
	}
...
extcommunity_list_show (struct vty *vty, struct community_list *list)
{
  ...
  for (entry = list->head; entry; entry = entry->next)
	...
	{
	  if (all_digit (list->name))
	    LOG_ERROR("Extended community %s list %s%s",
		     entry->style == EXTCOMMUNITY_LIST_STANDARD ?
		     "standard" : "(expanded) access",
		     list->name);
	  else
	    LOG_ERROR("Named extended community %s list %s%s",
		     entry->style == EXTCOMMUNITY_LIST_STANDARD ?
		     "standard" : "expanded",
		     list->name);
	}
       ...
       "Extcommunity-list name\n")
{
  ...
  if (! list)
    {
      LOG_ERROR("%% Can't find extcommunity-list");
      return CMD_WARNING;
    }
...
community_list_config_write (struct vty *vty)
{
  ...
  for (list = cm->num.head; list; list = list->next)
    for (entry = list->head; entry; entry = entry->next)
      {
	LOG_ERROR("ip community-list %s %s %s",
  ...
  for (list = cm->str.head; list; list = list->next)
    for (entry = list->head; entry; entry = entry->next)
      {
	LOG_ERROR("ip community-list %s %s %s %s",
		 entry->style == COMMUNITY_LIST_STANDARD
		 ? "standard" : "expanded",
		 list->name, community_direct_str (entry->direct),
		 community_list_config_str (entry));
  ...
  for (list = cm->num.head; list; list = list->next)
    for (entry = list->head; entry; entry = entry->next)
      {
	LOG_ERROR("ip extcommunity-list %s %s %s%s",
		 list->name, community_direct_str (entry->direct),
		 community_list_config_str (entry));
	write++;
      }
  for (list = cm->str.head; list; list = list->next)
    for (entry = list->head; entry; entry = entry->next)
      {
	LOG_ERROR("ip extcommunity-list %s %s %s %s%s",
		 entry->style == EXTCOMMUNITY_LIST_STANDARD
		 ? "standard" : "expanded",
		 list->name, community_direct_str (entry->direct),
		 community_list_config_str (entry));
