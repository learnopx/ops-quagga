...
def apply_bgp_config(step):
    ...
    for bgp_cfg in bgpconfigarr:
        ...
        if i == 0:
            ...
            cfg_array.append("ip extcommunity-list BGP_IN permit 2:0")
            cfg_array.append("ip extcommunity-list BGP_IN deny 3:0")
            step("### Applying route-map configurations for BGP1")
            cfg_array.append("route-map BGP_RMAP permit 10")
            cfg_array.append("match extcommunity BGP_IN")

        if i == 1:
            cfg_array.append("route-map BGP_2 permit 10")
            cfg_array.append("set extcommunity rt 2:0")
            cfg_array.append("set extcommunity soo 4:0")

        if i == 2:
            cfg_array.append("route-map BGP_3 permit 10")
            cfg_array.append("set extcommunity rt 3:0")
...
def test_bgp_ft_ip_extcommunity_list(topology, step):
    global switches
