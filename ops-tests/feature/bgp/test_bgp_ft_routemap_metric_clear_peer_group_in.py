...
def clear_soft_in_peer_group(dut, group):
    enterconfigshell(dut)

    dut("do clear bgp peer-group " + group + " soft in")
    exitcontext(dut)
...
def verify_routemap_set_metric_clear_soft_in_peer_group(step, switch1,
                                                        switch2, switch3):
    step("\n\n########## Verifying route-map set metric test 1"
         " for clear soft in peer group command##########\n")
    ...
    configure_neighbor_rmap_in(switch2, AS_NUM2, "pGroup", AS_NUM1, "BGP_IN")

    clear_soft_in_peer_group(switch2, "pGroup")
    ...
    found = wait_for_route(switch2, network1,
                           next_hop1)

    assert found, "Could not find route (%s -> %s) on %s" % \
        (network1, next_hop1, switch2.name)

    found = wait_for_route(switch2, network3,
                           next_hop3)

    assert found, "Could not find route (%s -> %s) on %s" % \
        (network3, next_hop3, switch2.name)
    ...
    assert (set_metric_flag_1 is True), "Failure to verify clear bgp" \
                                        " peer group" \
                                        " soft in command on peer 2 for " \
                                        "neighbor %s" % IP_ADDR1
    assert (set_metric_flag_3 is True), "Failure to verify clear bgp peer" \
                                        " group" \
                                        " soft in command on peer 2 for" \
                                        " neighbor %s" % IP_ADDR3
    step("### \"clear bgp peer group soft in\" validated succesfully "
...
@mark.timeout(600)
def test_bgp_ft_routemap_metric_clear_peer_group_in(topology, step):
    ...
    verify_routemap_set_metric_clear_soft_in_peer_group(step, ops1,
                                                        ops2, ops3)
