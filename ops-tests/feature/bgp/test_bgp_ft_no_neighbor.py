...
def test_bgp_ft_no_neighbor(topology, step):
    global switches
    ...
    unconfigure_neighbor_bgp(step)

    # Since neighbor configuration on switch 1 was removed, verify
    # that the route is no longer there on switch 2.
    verify_bgp_route_removed(step, switches[1], bgp1_network, bgp1_router_id)
