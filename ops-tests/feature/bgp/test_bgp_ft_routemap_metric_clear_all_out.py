...
def clearsoftoutall(dut):
    dut("clear bgp * soft out")
...
def verify_routemap_set_metric_clear_soft_out_all(sw1, sw2, sw3):
    print("\n\n########## Verifying route-map set metric test 1"
          " for clear soft out all command##########\n")
    metric = "12"
    ...
    assert set_metric_flag_1, "Failure to verify clear bgp all soft out \
                               command on peer 2 for neighbor %s" % IP_ADDR1
    print("### 'clear bgp all soft out' validated succesfully "
    ...
    assert set_metric_flag_3, "Failure to verify clear bgp all soft out \
                               command on peer 2 for neighbor %s" % IP_ADDR3
    print("### 'clear bgp all soft out' validated succesfully "
          "on peer 2 for neighbor %s###\n" % IP_ADDR3)


def verify_routemap_set_metric_clear_soft_in_all(sw1, sw2, sw3):
    print("\n\n########## Verifying route-map set metric test 1"
          " for clear soft in all command##########\n")
    metric = "18"
    ...
    assert set_metric_flag_1, "Failure to verify clear bgp all soft in command\
                               on peer 2 for neighbor %s" % IP_ADDR1
    assert set_metric_flag_3, "Failure to verify clear bgp all soft in command\
                               on peer 2 for neighbor %s" % IP_ADDR3
    print("### 'clear bgp all soft in' validated succesfully "
...
def test_bgp_metric_clear_all_out_configuration(topology, step):
    ...
    verify_routemap_set_metric_clear_soft_out_all(sw1, sw2, sw3)
