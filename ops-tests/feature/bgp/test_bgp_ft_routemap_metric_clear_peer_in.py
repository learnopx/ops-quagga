...
def clear_soft_in_neighbor(dut, neighbor_addr):
    dut("clear bgp " + neighbor_addr + " soft in")
...
def verify_routemap_set_metric_clear_soft_in_peer(topology, step):
    step("\n\n########## Verifying route-map set metric test 1"
         " for clear soft in peer command##########\n")
    ...
    configure_neighbor_rmap_in(switch2, as_num2, ip_addr1, as_num, "BGP_IN")

    step("Configuring neighbor route-map on SW2 for peer 3")
    configure_neighbor_rmap_in(switch2, as_num2, ip_addr3, as_num3, "BGP_IN")

    clear_soft_in_neighbor(switch2, ip_addr1)
    clear_soft_in_neighbor(switch2, ip_addr3)
    ...
    found = wait_for_route(switch2, network1, next_hop1)

    assert found, "Could not find route (%s -> %s) on %s" % \
        (network1, next_hop1, "ops2")

    found = wait_for_route(switch2, network3, next_hop3)

    assert found, "Could not find route (%s -> %s) on %s" % \
        (network3, next_hop3, "ops2")
...
def test_bgp_ft_routemap_metric_clear_peer_in(topology, step):
    configure(topology, step)
    verify_routemap_set_metric_clear_soft_in_peer(topology, step)
